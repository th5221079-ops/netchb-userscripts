...
// @grant        GM_log
// @grant        unsafeWindow
// @updateURL   https://github.com/th5221079-ops/netchb-userscripts/releases/latest/download/lacey_act_auto_disclaim.release.user.js
// @downloadURL https://github.com/th5221079-ops/netchb-userscripts/releases/latest/download/lacey_act_auto_disclaim.release.user.js
// ==/UserScript==
(function () {
  'use strict';
  ...
  const banner=(m,ok=true)=>{
    ...
    el.__tm=setTimeout(()=>el.remove(),5000);
  };
  ...
  // 如果本页会话之前已经按 F10 关掉过，直接退出，不再提示
  try{
    if(sessionStorage.getItem(STOP_KEY)==='1'){ return; }
  }catch{}

  // 进入页面时的右下角提示气泡：告诉用户可以按 F10 关闭脚本
  banner('Lacey Act 自动脚本已启用，本页按 F10 可关闭脚本。', true);
  ...
  if(!tr){
    banner('未找到 “PGA: Lacey Act” 行（仅当未完成时才执行）', false);
    return;
  }
  ...
  if(!res.ok){
    banner('未能触发 Disclaim（将继续尝试选择 A）', false);
  }

  setTimeout(()=>{
    if(!stopped) waitAndSelectA(info.idStr);
  }, 150);
})();
